...
export function stepProjectiles(dt, state){
  for(const p of projectiles){ p.x+=p.vx*dt; p.y+=p.vy*dt; p.life-=dt; if(p.life<=0) p.dead=true;
    for(const e of breads){ if(!e.alive) continue; if(Math.hypot(p.x-e.x,p.y-e.y)<=e.r){
      damageBread(e,p.dmg,state);
      if(p.splash>0){ for(const e2 of breads){ if(!e2.alive) continue; const d=Math.hypot(p.x-e2.x,p.y-e2.y); if(d<=p.splash && e2!==e){ damageBread(e2,p.splashDmg,state); } } }
