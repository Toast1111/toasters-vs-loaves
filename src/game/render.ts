...
export function drawScene(ctx, state){
  ...
  for(const e of breads){ if(!e.alive) continue; const hp=e.hp/e.maxHp; ctx.save(); ctx.translate(e.x,e.y);
    ctx.fillStyle="#c58a55"; roundedRect(ctx,-14,-10,28,20,8);
    ctx.fillStyle="#8c5a2f"; roundedRect(ctx,-14,-12,28,8,6);
    ctx.fillStyle="#000000aa"; ctx.fillRect(-14,-16,28,3); ctx.fillStyle= hp>0.4? '#7bd88f' : '#ff6b6b'; ctx.fillRect(-14,-16,28*hp,3);
  ctx.restore(); }
  for(const t of state.toasters){
    ...
    if(state.showRanges && (!state.selected || state.selected===t.id)){ ctx.strokeStyle="#3a445f66"; ctx.beginPath(); ctx.arc(0,0,t.range,0,Math.PI*2); ctx.stroke(); }
    ctx.restore();
  }
  for(const p of projectiles){ ctx.fillStyle="#ffd166"; ctx.beginPath(); ctx.arc(p.x,p.y,3,0,Math.PI*2); ctx.fill(); }
  for(const pr of particles){ ctx.fillStyle="#e7c08a88"; ctx.fillRect(pr.x,pr.y,2,2); }
