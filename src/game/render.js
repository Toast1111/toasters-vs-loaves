...
export function drawScene(ctx, state) {
    ...
    for (const e of breads) {
        ...
        if (e.type === 'sourdough_boss') {
            ...
            roundedRect(ctx, -e.r, -e.r * 0.8, e.r * 2, e.r * 1.6, e.r * 0.4);
            ctx.fillStyle = "#b8956a";
            roundedRect(ctx, -e.r * 0.8, -e.r * 0.6, e.r * 1.6, e.r * 0.8, e.r * 0.3);
            ...
            if (hp < 0.7) {
                ...
                ctx.moveTo(e.r * 0.3, -e.r * 0.4);
                ctx.lineTo(e.r * 0.6, e.r * 0.1);
                ctx.stroke();
            }
        }
            ...
            roundedRect(ctx, -e.r * 1.2, -e.r * 0.6, e.r * 2.4, e.r * 1.2, e.r * 0.2);
            ctx.fillStyle = "#d4b087";
            roundedRect(ctx, -e.r, -e.r * 0.4, e.r * 2, e.r * 0.6, e.r * 0.15);
            // Speed burst effect
            if (e.lastSpeedBurst < 0) {
                ...
                for (let i = 0; i < 5; i++) {
                    ctx.beginPath();
                    ctx.arc(0, 0, e.r + i * 3, 0, Math.PI * 2);
                    ctx.stroke();
                }
            }
        }
            ...
            roundedRect(ctx, -e.r * 0.6, -e.r * 0.6, e.r * 1.2, e.r * 1.2, 3);
        }
        else {
            // Regular bread
        ...
        ctx.fillRect(-barWidth / 2, -e.r - barHeight * 2, barWidth, barHeight);
        ctx.fillStyle = hp > 0.4 ? '#7bd88f' : '#ff6b6b';
        ctx.fillRect(-barWidth / 2, -e.r - barHeight * 2, barWidth * hp, barHeight);
    ...
    for (const t of state.toasters) {
        ...
        if (t.type === 'airfryer') {
            ...
            ctx.fillStyle = "#c67e3b";
            roundedRect(ctx, -18, -16, 36, 32, 8);
            ctx.fillStyle = "#e2945a";
            roundedRect(ctx, -16, -14, 32, 12, 6);
            ctx.fillStyle = "#8b4513";
            roundedRect(ctx, -12, -8, 24, 8, 4);
            // Heat wave effect
            ctx.strokeStyle = "#ff6b35aa";
            ctx.lineWidth = 1;
            for (let i = 0; i < 3; i++) {
                ctx.beginPath();
                ctx.arc(0, 0, 12 + i * 4, 0, Math.PI * 2);
                ctx.stroke();
            }
        }
        else if (t.type === 'industrial') {
            ctx.fillStyle = "#778899";
            roundedRect(ctx, -20, -18, 40, 36, 8);
            ctx.fillStyle = "#c0c0c0";
            roundedRect(ctx, -18, -16, 36, 14, 6);
            ctx.fillStyle = "#1a1a1a";
            roundedRect(ctx, -14, -10, 28, 12, 4);
            // Chrome shine effect
            ctx.fillStyle = "#ffffff44";
            roundedRect(ctx, -16, -14, 8, 3, 2);
        }
        else if (t.type === 'chef') {
