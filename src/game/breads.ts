...
export function stepBreads(dt, state){
  for(const e of breads){
    ...
    if(!target){ e.alive=false; state.lives-=1; document.getElementById('lives').textContent=state.lives; if(state.lives<=0) { state.running=false; } continue; }
    const dx=target.x-e.x, dy=target.y-e.y; const d=Math.hypot(dx,dy);
    if(d<2){ e.wpt++; continue; }
    const vx=dx/d*e.speed, vy=dy/d*e.speed; e.x+=vx*dt; e.y+=vy*dt;
  }
}
export function damageBread(e, dmg, state){
  e.hp-=dmg; if(e.hp<=0){ e.alive=false; const bonus=Math.round(e.bounty*state.global.bounty); state.coins+=bonus; document.getElementById('coins').textContent=state.coins; }
}
let _id=0;
