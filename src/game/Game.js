...
import { fireFrom, stepProjectiles } from "./projectiles";
import { stepParticles, spawnMuzzleFlash } from "./particles";
import { getTowerBase } from "./towers";
import { spawnBread, breads, stepBreads } from "./breads";
...
export class Game {
    ...
    init() {
        ...
        document.addEventListener('keydown', e => {
            if (e.target.tagName === 'INPUT')
                return; // Don't trigger during typing
    ...
    tryPlaceTower(x, y, multi = false) {
        if (!isBuildable(x, y)) {
            UI.float(this, x, y, 'Not on loaf path!', true);
            return;
        }
        const type = this.state.placing;
        if (!type)
            return;
        if (this.state.coins < type.cost) {
            UI.float(this, x, y, 'Not enough coins', true);
            return;
        }
        for (const t of this.state.toasters) {
            if (Math.hypot(x - t.x, y - t.y) < 36) {
                UI.float(this, x, y, 'Too close to another toaster', true);
                return;
            }
        }
        const inst = this.makeToaster(type, x, y);
        this.state.toasters.push(inst);
        this.state.coins -= type.cost;
        UI.sync(this);
    ...
    getTowerCost(t) { const base = getTowerBase(t.type).cost; let spent = 0; const ups = getTowerBase(t.type).upgrades; for (let i = 0; i < t.level; i++)
        spent += ups[i].cost; return base + spent; }
    ...
    update(dt) {
        ...
        if (this.state.waveInProgress) {
            ...
            if (this.state.spawnTimer <= 0 && this.state.waveQueue.length) {
                spawnBread(this.state.waveQueue.shift());
            ...
            if (!this.state.waveQueue.length && breads.every(b => !b.alive)) {
                ...
                this.state.ap += 1;
                this.state.coins += 50 + this.state.wave * 10;
                UI.sync(this);
