...
export const UI = {
    bind(game) {
        ...
        document.getElementById('startBtn').onclick = () => { if (game.state.betweenWaves)
            game.startWave(); };
    ...
    log(msg) { const el = document.getElementById('log'); const atBottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 4; el.insertAdjacentHTML('beforeend', `<div>• ${msg}</div>`); if (atBottom)
    ...
    refreshTech(game) {
      ...
      <button class="buy btn small">${`Buy (${t.cost} AP)`}</button>`;
            node.querySelector('.buy').onclick = () => {
                if (game.state.ap < t.cost) {
                    this.log('Not enough AP. Survive more waves!');
                    return;
                }
                game.state.ap -= t.cost;
                this.sync(game);
                t.action(game);
                this.log(`Tech unlocked: ${t.name}`);
                this.refreshTech(game);
    ...
    `;
        if (canUpgrade) {
            document.getElementById('upgradeBtn').onclick = () => {
                if (game.state.coins < up.cost)
                    return;
                game.state.coins -= up.cost;
                this.sync(game);
                up.effect(t);
                t.level++;
                this.float(game, t.x, t.y, 'Upgrade!');
                this.updateInspect(game);
                this.refreshCatalog(game);
            };
        }
